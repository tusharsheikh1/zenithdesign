...
const BackgroundGrid = () => (
  <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
    ...
    <div className="absolute top-[-20%] left-[-10%] w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-indigo-500/20 rounded-full blur-[80px] md:blur-[120px] mix-blend-multiply dark:mix-blend-screen animate-pulse" />
    <div className="absolute bottom-[-20%] right-[-10%] w-[250px] md:w-[500px] h-[250px] md:h-[500px] bg-orange-500/20 rounded-full blur-[80px] md:blur-[100px] mix-blend-multiply dark:mix-blend-screen" />
  </div>
);
...
const FloatingBadge = ({ text, delay, className }: { text: string; delay: number; className?: string }) => (
  <motion.div
    ...
    className={`absolute z-20 px-3 py-1.5 rounded-lg text-[10px] md:text-xs font-mono font-bold shadow-xl backdrop-blur-md border border-white/20 dark:border-white/10 ${className}`}
  >
    {text}
  </motion.div>
);
...
const CodeWindow = () => {
  return (
    <motion.div
      ...
      className="relative w-full max-w-[90vw] md:max-w-lg mx-auto z-10"
    >
      {/* Floating Badges: Adaptive Colors & Mobile Position Fixes */}
      <FloatingBadge 
        text="🚀 SEO Ready" 
        delay={2.5} 
        className="top-[-15px] right-0 md:top-[-20px] md:right-[-10px] rotate-3 md:rotate-6 
      ...
      <FloatingBadge 
        text="⚡ Fast" 
        delay={2.8} 
        className="bottom-[15%] left-0 md:bottom-[20%] md:left-[-20px] -rotate-3 md:-rotate-6 
      ...
      <div className="relative bg-[#1e293b] dark:bg-[#0f172a]/90 backdrop-blur-xl border border-slate-300 dark:border-slate-700/50 rounded-2xl shadow-2xl dark:shadow-indigo-500/10 overflow-hidden ring-1 ring-white/10">
        ...
        {/* Code Content */}
        <div className="p-4 md:p-6 font-mono text-xs md:text-sm leading-6 md:leading-7 text-slate-300 overflow-x-auto">
...
const Hero = () => {
  return (
    <section id="home" className="relative pt-28 pb-16 md:pt-36 md:pb-24 lg:pt-48 lg:pb-32 overflow-hidden bg-white dark:bg-[#020617] transition-colors duration-500">
      ...
      <div className="container relative z-10 mx-auto px-6 md:px-12 grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
        ...
        <motion.div 
          ...
          initial="hidden"
          animate="visible"
          className="text-center lg:text-left max-w-2xl mx-auto lg:mx-0 order-1"
        >
          <motion.div variants={fadeInUp} className="mb-6 flex justify-center lg:justify-start">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 text-[10px] md:text-xs font-semibold uppercase tracking-wider shadow-sm transition-colors">
          ...
          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
            ...
            {/* Primary Button */}
            <a href="#pricing" className="relative inline-flex h-12 md:h-14 overflow-hidden rounded-full p-[2px] focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 focus:ring-offset-slate-50 group shadow-lg shadow-indigo-500/30">
          ...
          <motion.div variants={fadeInUp} className="mt-8 md:mt-10 flex items-center justify-center lg:justify-start gap-4 text-xs md:text-sm text-slate-500 dark:text-slate-500 font-medium transition-colors">
            <div className="flex -space-x-2">
               {[1,2,3].map(i => (
                 <div key={i} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-slate-200 dark:bg-slate-800 border-2 border-white dark:border-[#020617] flex items-center justify-center text-[10px] transition-colors">👤</div>
               ))}
            </div>
        ...
        <div className="relative perspective-[2000px] order-2 mt-8 lg:mt-0">
          {/* Decorative Blob */}
